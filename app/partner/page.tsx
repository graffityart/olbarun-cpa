...
export default async function PartnerPage() {
  ...
  const [earningSummary] = await db.select({ today: sql<number>`coalesce(sum(case when ${earnings.createdAt} >= ${today} then ${earnings.amount} else 0 end),0)::int`, confirmed: sql<number>`coalesce(sum(${earnings.amount}),0)::int`, available: sql<number>`coalesce(sum(case when ${earnings.status} = 'AVAILABLE' then ${earnings.amount} else 0 end),0)::int` }).from(earnings).where(eq(earnings.partnerId, partnerId));
