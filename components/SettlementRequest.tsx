...
export default function SettlementRequest({ available }: { available: number }) {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    const payload = { amount: Number(f.get("amount") || 0), bankName: f.get("bankName"), accountNumber: f.get("accountNumber"), accountHolder: f.get("accountHolder") };
  ...
  return <form onSubmit={submit} className="panel card"><h2>정산 신청</h2><p className="muted">현재 출금가능금액 {available.toLocaleString("ko-KR")}원</p><div className="form-grid"><label>신청금액 *<input name="amount" type="number" min="1" max={available} required /></label><label>은행명 *<input name="bankName" required /></label><label>계좌번호 *<input name="accountNumber" required /></label><label>예금주 *<input name="accountHolder" required /></label></div>{message&&<p className="form-message">{message}</p>}<div className="form-actions"><button disabled={loading||available<=0}>{loading?"신청 중...":"정산 신청"}</button></div></form>;
}
